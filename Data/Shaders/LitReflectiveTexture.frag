...
void main()
{
	...
	vec3 I = normalize(v_ObjectPosition - v_CameraPosition);
	vec3 R = reflect(I, normal);
	vec3 skyBoxColor = textureCube(u_Skybox, R).rgb;

	// Get the direction to the camera
	vec3 viewDirection = normalize(v_CameraPosition - v_ObjectPosition);

	// Get the objects color
	vec3 diffuseColor = saturate(texture2D(u_Material.diffuse, v_UV).xyz * skyBoxColor);
