...
SoundPlayer::SoundPlayer()
{
    ...
    m_pChannel = new SoundChannel*[MAX_CHANNELS];

    for (int i = 0; i < MAX_CHANNELS; i++)
    {
        m_pChannel[i] = new SoundChannel(m_pEngine);
    }
}
...
int SoundPlayer::PlaySound(SoundObject* pSoundObject)
{
    ...
    for (int i = 0; i < MAX_CHANNELS; i++)
    {
        if (m_pChannel[i]->GetState() == SoundChannel::SoundChannelState_Free)
        {
            m_pChannel[i]->PlaySound(pSoundObject);
            return i;
        }
    }

    double oldestChannelTime = DBL_MAX;
    int oldestChannelIndex = -1;

    for (int i = 0; i < MAX_CHANNELS; i++)
    {
        if (m_pChannel[i]->GetTimePlaybackStarted() < oldestChannelTime)
        {
            oldestChannelTime = m_pChannel[i]->GetTimePlaybackStarted();
            oldestChannelIndex = i;
        }
    }

    m_pChannel[oldestChannelIndex]->PlaySound(pSoundObject);
...
{
    m_pChannel[channelIndex]->StopSound();
}